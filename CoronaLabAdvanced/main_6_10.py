...
# в гистограмму входят ВСЕ группы КРОМЕ САМОЙ МАССОВОЙ С УЧЁТОМ ВЕСА ЭТОЙ ГРУППЫ
...
for i in range(len(B66_groups)):
    for j in B66_groups[i]:
        data[i].append([j, B66_groups[i][j]['count']])
...
for i in range(len(data)):
    ...
    for a66 in data[i]:
        if a66[1] < 300000:
            for j in range(len(a66[0])):
            #if MASS:
                data_freqs[i][j][a66[0][j]] += a66[1]
            #else:
                #data_freqs[i][j][a66[0][j]] += 1
...
def get_hist(segment_num, freqs, NAME):
    ...
    if segment_num == 1:
        plt.title('Встречаемость нуклеотида  на позиции в лидер-отрезке')
    else:
        plt.title('Встречаемость нуклеотида группы на позиции в отрезке №' + str(segment_num-1))
...
for i in range(11):
    get_hist(i+1, data_freqs, 'mass_mutants_'+str(i))