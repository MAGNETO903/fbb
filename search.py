...
path_to_file = str(input("Введите имя (или путь до файла без приставки .gb) вируса (например, SARS): "))#"HCoV_gb.gb"
#path_to_file = 'SARS'
...
for word in words_in_leader:
    ...
    for j in range(len(u_markup)):
        matches = [{"num": j+1, "local_pos": m.start(), "global_pos": m.start()+u_markup[j][0]} for m in re.finditer(word, str(genom)[u_markup[j][0]:u_markup[j][1]])]
        if len(matches) > 1:
            strange = True
            matches_num+=1
        elif len(matches) == 1:
    ...
    for j in range(len(f_markup)):
        matches = [{"num": j+1, "local_pos": m.start(), "global_pos": m.start()+f_markup[j][0]} for m in re.finditer(word, str(genom)[f_markup[j][0]:f_markup[j][1]])]
        f_matches += matches

    l_matches = [{"local_pos": m.start(), "global_pos": m.start()+leader_markup[0]} for m in re.finditer(word, str(genom)[leader_markup[0]:leader_markup[1]])]
...
for i in range(len(words_candidates)):
    ...
    print(pd.DataFrame(data=matches))
